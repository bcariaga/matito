...
export const StyledBalloon = styled.div`
  // top: 100px;
  background-color: ${(props: StyledProps) => colorMaps[props.color]};
  display: ${(props: StyledProps) => (props.show ? "block" : "none")};
  visibility: ${(props: StyledProps) => (props.visible ? "visible" : "hidden")};
  left: ${(props: StyledProps) => `${props.animate.left}vw`};
  transition: transform 0.5s ease;
  z-index: 10;
  animation: ${(props: StyledProps) =>
    (props.animate.left ?? 0) > 50
      ? balloonsRightUpKFAnimation(props.animate)
      : balloonsLeftUpKFAnimation(props.animate)} ease-in-out ${(props) =>
  props.animate.loop ? "infinite" : "1"};
  // animation-duration: 3s;
  animation-duration: ${(props: StyledProps) => `${props.animate.duration}s`};
  animation-delay ${(props: StyledProps) => `${props.animate.delay}s`};
  animation-fill-mode: ${(props: StyledProps) =>
    props.animate.hangOnTop ? "forwards" : "none"};
